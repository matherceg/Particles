...
% Load Mag model
modelName = 'E:\Missions\JUNO\MagneticModel\JupiterGlobalField\JRM09_20.mat' ;
load(modelName);
% Max degree
n_max=10;
% Extract model up to the highest degree
model=JRM09(1:n_max*(n_max+2));
...
mat_Mag_2_S3 = ea2matr(2,3,2,0, 163.39, 10.31,'deg')';


% Jupiter
% Particle bouncing period (Guio et al., 2020)
tempE=Ek_mev*(Ek_mev+2*E0_mev);
Tb = 0.954*(Erel_mev/sqrt(tempE))*Inj_R*(1.15-0.29*sin(pitchAngle*pi/180)-0.04*Inj_R*sin(pitchAngle*pi/180));

% Particle averaged drift period
Td = 1272.67*(Erel_mev/tempE)*1/Inj_R*(0.55-0.55*sin(pitchAngle*pi/180)+0.10*Inj_R*sin(pitchAngle*pi/180)-2.54*10^-3*Inj_R^2*sin(pitchAngle*pi/180))^-1;
...
% disp(['Gyro period = ',num2str(Tg),' sec']);
...
% Convert injected particle coordinates from JRM09 to S3 frame
% Injected particle is in the Magnetic Equator
r_S3  = (mat_Mag_2_S3*[Inj_R 0 0]')';

% Convert to spherical S3 coordinates
[lon,lat,rad] = cart2sph(r_S3(1),r_S3(2),r_S3(3));
Inj_S3_sph=[lon*180/pi,lat*180/pi,rad];

% Injected particle in S3
r0= Inj_S3_sph(3);
lon0=Inj_S3_sph(1);
lat0=Inj_S3_sph(2);
...
% Current sheet (in Gauss)
[dbr, dbt, dbp, rho, z, angl ] = shtfld(r0,(90-lat0)*pi/180,lon0*pi/180,7.9,52.2, 3.6,0.001395, 9.1, 205.8);

% JRM09 + current sheet magnetic field (in Gauss)
Br=(b_r*10^-5)+dbr;
Btheta=(b_theta*10^-5)+dbt;
Bphi=(b_phi*10^-5)+dbp;

% JRM09 + current sheet (in Tesla)
Br=Br*10^-4;
Btheta=Btheta*10^-4;
Bphi=Bphi*10^-4;
...
% Rotation of B vector from Local to S3
...
disp(['Gyro radius = ',num2str(rG),' km']);

% B at the Mirror point in T
Bm = B/(sin(pitchAngle*pi/180)^2);
% B at the Mirror point in Gauss
Bm = Bm * 10^4;



OnlyPlanetTrack=0;
% Track and plot magnetic field line
r_Mag  = (mat_Mag_2_S3'*r_S3')';
% Track the field line
tflMaxDist=Inj_R*2;
incrementTangent=0.01;
tfl=[];
TrackMagFieldLine_Plot

% Convert Tracked field line in JRM09
tfl_JRM09  = (mat_Mag_2_S3'*tfl')';

[~,idx]=min(abs(B_mfl-Bm));
tflM=tfl_JRM09(idx,:);

[long_sph,lat_sph,r_sph] = cart2sph(tflM(:,1),tflM(:,2),tflM(:,3));

disp(['Mirror point = ',num2str(abs(lat_sph*180/pi)),' deg']);
